...
from .adb import get_rotation_and_size, launch_scrcpy, screencap
...
class MainWindow(QMainWindow):
    ...
    def _update_title(self) -> None:
        name = self.scene.name or "(無題)"
        path = self.current_scene_path or "未保存"
        self.setWindowTitle(f"nightcrows scene editor — {name} — {path}")
    ...
    def _reload_device_combo(self) -> None:
        prev_serial = self._current_serial()
        self.device_combo.blockSignals(True)
        self.device_combo.clear()
        for d in self.settings.devices:
            self.device_combo.addItem(f"{d.label}  ({d.serial})", d.serial)
        if prev_serial:
    ...
    def _compute_view(self, selected_idx: int | None) -> tuple[str | None, list[tuple[int, int, int, bool]]]:
        """選択行に応じて (表示すべきスナップパス, マーカー一覧) を返す。

        マーカー: (番号, logical x, logical y, ハイライトか)
        番号はそのスナップグループ内のタップの 1 始まり連番。
        """
        # 各ステップが属するスナップグループを割り当て
        snapshots: list[str] = []
        group: list[int] = []
        current = -1
    ...
    def _start_recording(self) -> None:
        ...
        # スナップが1枚も無い場合は警告（座標のみ記録、マーカー表示不可）
    ...
    def _write_scene(self, path: str) -> None:
        self.scene.serial = self._current_serial()
        try:
    ...
    def _load_scene(self) -> None:
        ...
        self._pixmap_cache.clear()
        self.scene_name_edit.setText(self.scene.name)
        if self.scene.serial:
            idx = self.device_combo.findData(self.scene.serial)
            if idx >= 0:
                self.device_combo.setCurrentIndex(idx)
        self._refresh_step_list()
        self._update_title()
