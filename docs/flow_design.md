...
### 昨日フォールバック（PC 版 2026-05-28）

深夜帯（例: 03:35）に発火した場合、当日の有効エントリが「現在時刻より前」にまだ一つも無いケースがある。
従来は `None` を返してスキップしていたが、`pc/gui/pc_flow.py: last_due_scenes` を以下に拡張：

1. まず今日 (`time < now`) で候補を探す
2. 当日候補が空なら**昨日**の有効エントリ全件（`time < "99:99"`）を再走査
3. 各日内で最も遅い時刻のエントリを採用

判定は `_due_scenes_on(flow, target_date, max_hm_exclusive)` ヘルパーに集約。`daily` / `weekly`（曜日チェック）/ `once`（日付チェック）すべて対象日付で判定される。

```
例: 5/28 03:35 死亡ウォッチャー発火、daily 06:10 / weekly 月 22:30 等のみ
→ 今日候補ゼロ（06:10 は未来、22:30 は月曜のみ）
→ 昨日 5/27 (水曜) の有効エントリを走査
→ 昨日の daily 06:10 を採用 → 06:10 シーンを実行
```
