...
### 経験値計測パネル（`ExpMeterWidget`）

ウォッチャータブ下部に固定表示される計測パネル。フロー実行とは独立して動作する。

```
[ ▶ 計測開始 ]  [ 📍 領域設定 ]  [ 🔄 リセット ]   最終取得: 14:37

現在値:  73.45%    現在速度:  45.6 %/h    ↑ 平均より +8.5%/h
平均速度:  37.1 %/h    計測: 1h23m  17サンプル  最終: 14:37
```
...
| 項目 | 内容 |
|------|------|
| サンプリング間隔 | 5分（`_EXP_SAMPLE_INTERVAL_MS`） |
| 現在速度 | 直近3サンプル（約15分）の変化率（`_EXP_CURRENT_WINDOW = 3`） |
| 平均速度 | リセット後の全サンプルから算出 |
| 比較表示 | 現在速度 - 平均速度 > 0.1 → ↑緑 / < -0.1 → ↓赤 / それ以外 → ≈灰 |
| レベルアップ検知 | 前回値との差が -30% 未満の急落でLvUP判定。`(100 - prev) + raw` を加算 |
| OCR誤読対策 | 差が -30〜0% の場合は誤読とみなしてスキップ（累積値・prev_rawは更新しない） |
| リセット | 全サンプルをクリア・累積リセット。計測中なら即座に再開始 |

#### OCR

`flow_runner._preprocess_for_ocr` / `_ocr_digits_best` を流用。  
ホワイトリスト `0123456789.`、`--psm 7`（1行）で小数点付き%値を読む。  
スクショ取得はバックグラウンドスレッドで実行し、UIをブロックしない。

#### 設定ファイル

選択した領域を `exp_meter.json`（プロジェクトルート）に保存。再起動後も保持。
...
| 項目 | 決定 |
|------|------|
| LvUP回数の表示 | しない。誤検知と区別困難なため、速度計算への正しい加算のみに使う |
| スレッド | `threading.Thread`（daemon=True）でバックグラウンド取得 |
| データ永続化 | サンプル列はメモリのみ（再起動でリセット）。領域設定のみ保存 |
| 電源オフ対応 | なし。レベルが上がったらリセットを手動で押す運用 |
...
上部バーに「↑ PUSH」「↓ PULL」ボタンを設置し、git を使って2台の PC 間でシーン・フロー・ウォッチャーを同期する。

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│ デバイス: [ オフィス ▼ ]  [接続] [切断] [scrcpy起動] [📶診断] [🔧メンテ] [↑PUSH] [↓PULL] [⚙] │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
1. `git add scenes/ flows/ watchers/` — 3ディレクトリの変更をステージング
2. `git diff --cached --quiet` で差分確認 → 差分なしならコミットせず `git push` のみ実行
3. `git commit -m "sync YYYY-MM-DD HH:MM"` — タイムスタンプ入りのコミット
4. `git push` — リモート（GitHub）にプッシュ

### PULL の処理手順

1. `git pull` — リモートから取得してマージ
2. 完了後に `watcher_editor._load_from_dir()` でウォッチャーをリロード
3. `flow_editor.reload_current_flow()` で現在開いているフローをリロード（未開の場合はスキップ）
4. `flow_editor.refresh_watcher_tags()` でタグバーを更新

### 実装上の注意

- git コマンドはすべてバックグラウンドスレッドで実行し、結果を `sync_result_signal(bool, str)` 経由でメインスレッドに返す
- ログは `scene_editor._log()` に出力（シーン編集タブのログエリアに表示）
- ボタンは処理中に無効化して多重実行を防ぐ
- `git commit` が「nothing to commit」のケースはエラー扱いしない
